...
The DCI is calculated using detailed data from every event that happens during a game. Here’s how I compute the DCI:

1.  **Finding Defensive Actions:** First, I identify all defensive actions by a player. These include tackles, various duels (like fighting for the ball on the ground or in the air), interceptions (cutting off passes), clearances (kicking the ball away from danger), and blocks (stopping shots or passes). I only count actions that happen in the player's **defensive third** of the field, as these are usually the most critical for preventing goals.

2.  **Measuring Impact on Expected Threat (xT):** Next, I figure out how much each defensive action changes the "danger level" (or threat) posed by the opponent. I do this by calculating the **Expected Threat (xT)** both *before* and *after* the defensive action.

    * **xT Before:** This is my estimate of how dangerous the opponent's attack was *just before* the defensive action. It's calculated by combining two things:
    ...
    * **xT After:** This is my estimate of how dangerous the opponent's attack *still is* right *after* the defensive action.
        ...
        * If the opponent keeps the ball, I estimate the xT of their next two touches.
...
3.  **Calculating the Difference:** For every defensive action, I subtract the "xT After" from the "xT Before" (`xT_before - xT_after`). A larger positive difference means the defensive action was more valuable because it significantly reduced the opponent's threat. This difference is the "threat reduction" for that action.

4.  **Combining Results:** Finally, the DCI is calculated by adding up the total number of defensive actions and the total threat reduction caused by those actions. This gives me a single score for each player.

The formula for DCI is:

$$
DCI = \alpha \times \text{total defensive actions} + \beta \times \text{total xT difference}
$$
...
* $\alpha = 0.01$ and $\beta = 1.0$ are weighting factors. They ensure that both making many defensive actions (quantity) and making highly impactful ones (quality) contribute appropriately to a player's overall DCI score.
...
| **Player Name** | **Value End 2017/18 Season** | **Age End 2017/18 Season** | **Highest Value After** | **Difference (Formatted)** | **Rank of DCI in League** |
| :----------------- | :----------------------- | :--------------------- | :------------------ | :--------------------- | :-------------------- |
| Jules Koundé | €7,000,000 | 19 | €65,000,000 | €58.0m | 17 |
| Diego Carlos | €10,000,000 | 25 | €50,000,000 | €40.0m | 4 |
| Aaron Wan-Bissaka | €1,000,000 | 20 | €40,000,000 | €39.0m | 8 |
| Clément Lenglet | €25,000,000 | 22 | €60,000,000 | €35.0m | 18 |
| Joachim Andersen | €1,500,000 | 22 | €35,000,000 | €33.5m | 8 |
| Manuel Akanji | €22,000,000 | 22 | €45,000,000 | €23.0m | 16 |
| Berat Djimsiti | €1,000,000 | 25 | €22,000,000 | €21.0m | 9 |
| Benjamin Pavard | €30,000,000 | 22 | €50,000,000 | €20.0m | 19 |
| Philipp Lienhart | €2,500,000 | 21 | €20,000,000 | €17.5m | 1 |
| Lewis Dunk | €10,000,000 | 26 | €27,000,000 | €17.0m | 2 |
